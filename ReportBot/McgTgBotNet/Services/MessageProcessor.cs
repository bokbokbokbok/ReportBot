...
namespace McgTgBotNet.Services
{
    public class MessageProcessor : IMessageProcessor
    {
        ...
        private async Task ExecuteHandlerAsync(MessageRequest request)
        {
            ...
            {
                throw new NotImplementedException("Failed to retrieve message text.The message is null.");
            }

            var handler = _handlers.FirstOrDefault(predicate) ?? throw new InvalidDataException($"Invalid message input");
